...
their 7T protocol (TR 50 ms, flip 15°, TEs 4/12/20/28 ms), no multicompartment signal. Departures
from Ridani-as-published that remain on by default: the V1-to-segmentation registration shift
(a data bug fix; disable with --no-v1-shift), the matched spin-echo acquisition (additive data the
benchmark needs; the SE model is plain mono-exponential R2 unless --multicompartment), and the
native-resolution packaging.
...
SEED = 42
B0 = 7.0
# The diffusion V1 map is misregistered relative to the segmentation by this integer voxel
# shift (its NIfTI header is a stale native-DWI affine, mutually inconsistent with the other
# maps, so affine-aware resampling is NOT the fix). Verified by mask cross-correlation:
# Dice(V1 support, BrainMask) 0.901 -> 0.924 after the shift. Integer shift specifically —
# interpolating eigenvectors across sign flips is ill-defined; no vector reorientation needed
...
def shift_int(arr, shift):
    """Integer zero-fill shift along the first 3 axes (no interpolation)."""
...
def main() -> None:
    ...
    ap.add_argument("--no-v1-shift", action="store_true",
                    help="skip the V1-to-segmentation registration shift (V1_SHIFT); for comparison only")
    ...
    # Register V1 to the segmentation (see V1_SHIFT) BEFORE it feeds anything — the χ- anisotropy
    # ground truth, theta and fiber_angle must all use the same registered fibre directions.
    v1_kw = {}
    if not args.no_v1_shift:
        v1_path = args.qsmf_data / "maps" / "V1.nii.gz"
        if v1_path.exists():
            v1_kw["v1"] = shift_int(nib.load(str(v1_path)).get_fdata().astype(np.float32), V1_SHIFT)
            print(f"  V1 registered to the segmentation: integer shift {V1_SHIFT} voxels")
    ...
    if args.maps_only:
        ...
        print(f"Downsampling maps to {VOXEL.tolist()} mm and simulating NATIVELY (no k-space crop)...")
        ...
        th1 = _ds_theta(theta, _ds) if theta is not None else None
        aff1 = q.resize(nib.Nifti1Image(chi_total, tp.nii_affine, tp.nii_header), VOXEL).affine
        def _w1(name, arr):
        ...
        q.generate_bids(
            ...
            chisep_signal=True, chisep_multicompartment=args.multicompartment, save_se=True,
            **extra,
        )
        ...
        if aniso and th1 is not None:  # optional DTI-orientation input, on the packed grid
