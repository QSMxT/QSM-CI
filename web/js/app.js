...
const METRICS = {
  ...
  region_linearity:{ label: "Region linearity", unit: "",  better: "lower",  dp: 3,
    desc: "|1 − slope| of mean reconstructed vs. true value fitted across every segmented region (DGM linearity is the same fit restricted to the six DGM nuclei). 0 = a perfectly linear response everywhere; a method recovering all regions at 80% of truth scores 0.2." },
  region_r2:       { label: "Region R²",        unit: "",  better: "higher", dp: 3,
    desc: "R² of that across-region fit: how much of the region-to-region variation the reconstruction actually explains. 1 = regional values track the ground truth exactly. A method can post a respectable xSIM and still sit near 0 here, which means its regional values are not tracking the truth at all." },
  region_bias:     { label: "Region bias",      unit: " ppm", better: "lower", dp: 4,
    desc: "Mean signed difference of the per-region means (reconstruction − truth), in ppm: the Bland–Altman bias. 0 is ideal; negative means systematic under-estimation. Signed, so sorting ascending puts the most strongly under-estimating methods first — read it as a diagnostic, not a ranking." },
