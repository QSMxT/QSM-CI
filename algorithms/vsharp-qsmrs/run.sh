#!/usr/bin/env bash
# QSM-CI submission — V-SHARP (bfr stage) via QSMxT / QSM.rs.
set -euo pipefail
...
if [ -f "$CFG" ]; then
  ...
  V=$(jq -r '.max_radius // empty' "$CFG"); [ -n "$V" ] && SET="$SET --max-radius $V"
  V=$(jq -r '.min_radius // empty' "$CFG"); [ -n "$V" ] && SET="$SET --min-radius $V"
fi
