# Building matlab-sti-iharperella (compiled MATLAB → MATLAB Runtime)
...
Stage: `unwrap+bfr` — consumes `localfield` (ppm) + `mask` + `params`; runs `iHARPERELLA` → `chimap` (ppm).
Validated corr **0.992**, NRMSE 0.30 vs GT on `data/sim/dev`.

## 1. Fetch build-time deps (not committed; `shims/` IS committed — it's ours)
```bash
cp -r /path/to/NIfTI_20140122                          algorithms/matlab-sti-iharperella/nifti
cp -r /path/to/STISuite_V3.0/Core_Functions_P          algorithms/matlab-sti-iharperella/sti
```
...
parser, and iHARPERELLA doesn't need it.

## 2. Compile
```bash
cd algorithms/matlab-sti-iharperella
...
would otherwise miss the helper `.p` that `iHARPERELLA.p` calls at runtime). `-a shims` bundles the
...
`imclose`/`imopen`. `iHARPERELLA`→`FastQSM` uses `imclose` with a 1×3 line element; the morphology
...
```bash
docker build -t ghcr.io/astewartau/qsm-ci/matlab-sti-iharperella:v1 .   # FROM matlab-runtime:r2026a + COPY recon
docker push  ghcr.io/astewartau/qsm-ci/matlab-sti-iharperella:v1
```
...
(γ = 42.576e6) and passes `TE`,`B0` so `iHARPERELLA` undoes the same scaling → chi in ppm; the exact TE
cancels.
