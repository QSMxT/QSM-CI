#!/usr/bin/env bash
# QSM-CI submission — FANSI (nlTGV) (dipole stage) via QSMxT / QSM.rs.
set -euo pipefail
...
qsmxt invert fansi-tgv "$IN/localfield.nii.gz" -m "$IN/mask.nii.gz" -o "$OUT/chimap.nii.gz" --b0-direction $B0 $SET
