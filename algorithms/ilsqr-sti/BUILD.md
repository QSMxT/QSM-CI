...
Stage: `dipole` — consumes `localfield` (ppm) + `mask` + `params`; runs `QSM_iLSQR` → `chimap` (ppm).
...
parser, and iLSQR doesn't need it.
...
would otherwise miss the helper `.p` that `QSM_iLSQR.p` calls at runtime). `-a shims` bundles the
...
`imclose`/`imopen`. `QSM_iLSQR`→`FastQSM` uses `imclose` with a 1×3 line element; the morphology
...
```bash
docker build -t ghcr.io/astewartau/qsm-ci/matlab-sti-ilsqr:v1 .   # FROM matlab-runtime:r2026a + COPY recon
docker push  ghcr.io/astewartau/qsm-ci/matlab-sti-ilsqr:v1
```
...
(γ = 42.576e6) and passes `TE`,`B0` so `QSM_iLSQR` undoes the same scaling → chi in ppm; the exact TE
cancels.
