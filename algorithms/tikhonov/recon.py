#!/usr/bin/env python3
"""Tikhonov dipole inversion — QSM-CI `dipole` stage.

Reads /input/localfield.nii.gz (ppm) + mask + params, writes /output/chimap.nii.gz (ppm).
Closed-form L2 inversion: chi = F^-1{ D / (D^2 + alpha) * F{field} }.
"""
import json
import sys
...
def main(inp, out):
    ...
    chi = np.real(np.fft.ifftn(D / (D**2 + alpha) * np.fft.fftn(field))) * mask
