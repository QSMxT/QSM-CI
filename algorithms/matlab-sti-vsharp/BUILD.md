...
parser, and V-SHARP doesn't need it.
...
would otherwise miss the helper `.p` that `V_SHARP.p` calls at runtime). `-a shims` bundles the
...
`imclose`/`imopen`. `V_SHARP`→`FastQSM` uses `imclose` with a 1×3 line element; the morphology
...
```bash
docker build -t ghcr.io/astewartau/qsm-ci/matlab-sti-vsharp:v1 .   # FROM matlab-runtime:r2026a + COPY recon
docker push  ghcr.io/astewartau/qsm-ci/matlab-sti-vsharp:v1
```
...
(γ = 42.576e6) and passes `TE`,`B0` so `V_SHARP` undoes the same scaling → chi in ppm; the exact TE
cancels.
