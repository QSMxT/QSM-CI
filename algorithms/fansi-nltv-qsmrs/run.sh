#!/usr/bin/env bash
# QSM-CI submission — FANSI (nlTV) (dipole stage) via QSMxT / QSM.rs.
set -euo pipefail
...
qsmxt invert fansi "$IN/localfield.nii.gz" -m "$IN/mask.nii.gz" -o "$OUT/chimap.nii.gz" --b0-direction $B0 $SET
