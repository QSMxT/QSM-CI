...
def setup_environment(bids_dir, algo_dir, work_dir, copybids=False):
    ...
    # Copy the BIDS directory into the work_dir only if copybids is True
    if copybids:
...
def run_algo(client, docker_image, algo_name, bids_dir, work_dir, input_json, copybids=False):
    ...
    try:
        ...
        # Mount or copy the BIDS directory depending on the copybids flag
        volumes = {work_dir: {'bind': '/workdir', 'mode': 'rw'}}
        if not copybids:
            volumes[bids_dir] = {'bind': '/bids', 'mode': 'ro'}  # Mount BIDS directory read-only
        ...
        container = client.containers.create(
            ...
            volumes=volumes,
            working_dir='/workdir',
            command=["ln -s /bids /workdir/bids && ./main.sh"],  # Execute the main.sh script
...
def main():
    ...
    parser.add_argument('--copybids', action='store_true', help='Copies bids_dir to work_dir rather than using a mount')
    args = parser.parse_args()

    # Set up the environment once
    client, docker_image, algo_name, work_dir = setup_environment(args.bids_dir, args.algo_dir, args.work_dir, args.copybids)
    ...
    if not args.inputs_json:
        ...
        for input_json in parse_bids.parse_bids_directory(args.bids_dir):
            run_algo(client, docker_image, algo_name, args.bids_dir, work_dir, input_json, args.copybids)
    else:
